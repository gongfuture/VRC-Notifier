...
<p align="center"><strong>VRChat Friend Status Monitor & Notification Tool</strong></p>
...
<p align="center">A secure and reliable background monitoring tool that tracks friend online status changes in real-time</p>
...
- **NTFY Push** - Support open-source push platform NTFY, supports self-hosted and official server
...
⚠️ **Important Notice**: This tool **can run simultaneously with the VRChat game client**, but please note the following:

**Same Account Conflict Risk**:
- VRChat's security mechanism does not allow the **same account** to be logged in from the **same network/device** simultaneously
- If this tool and the game use the **same account** on the **same device or network**, it will cause account login conflicts

**Recommended Deployment**:
- Recommended: Use **different accounts** for the tool and the game
- Recommended: Deploy the tool on a **different device** (e.g., server, VPS, another computer)
- Recommended: Use **different networks** for the tool and the game (e.g., tool uses server network, game uses local network)
- Avoid: Running the tool and logging into the game on the same computer (same account)
...
|------------|------------------|-------------------|
| **Online** | Friend offline → online | Immediate (Email/Gotify/NTFY/Webhook) |
| **Offline** | Friend online → offline | Immediate (Email/Gotify/NTFY/Webhook) |
| **Status Change** | Status changes between states | Immediate (Email/Gotify/NTFY/Webhook) |
| **World Change** | Friend changes world within same status | Batch (Email/Gotify/NTFY/Webhook) |
...
- [NTFY](https://ntfy.sh/) - Open-source push service
